<?php
	...
	if (! empty($errors)) {
		...
		$query = 	"UPDATE `post` 
					SET `title` = '" . $mysqli->real_escape_string(strtolower(trim($_POST['form_data']['title']))) . "',
						`description` = '" . $mysqli->real_escape_string($_POST['form_data']['description']) . "',
						`category_id` = '" . $mysqli->real_escape_string($_POST['form_data']['category_id']) . "',
						`tags` = '" . $mysqli->real_escape_string(strtolower(trim($_POST['tagInput']))) . "',
						...
						`urlslug` = '" . $mysqli->real_escape_string(strtolower(trim($_POST['form_data']['urlslug']))) . "',
						`post_date` = '" . $mysqli->real_escape_string(strtolower(trim($_POST['form_data']['post_date']))) . "',
						`image` = '" . $mysqli->real_escape_string($ufile1_path) . "',
