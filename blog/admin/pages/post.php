...
$(window).load(function(){
	$("#table1").on("click", ".trash", function() {
		var id=$(this).data('id');
		$('#modalDelete').attr('href','?dispatch=post.delete&id='+id);
	})
});//]]> 
</script>
			...
			<div class="col-xs-12">
				...
				<div class="box box-primary">
					...
					<div class="box-header">
						<h3 class="box-title"><a href="?dispatch=post.add" class="btn btn-block btn-primary">Add New</a></h3>
					</div>
					...
					<div class="box-body table-rowdata">
						...
						<tbody>
							<?php
							$s_no = 1;
							$results = $mysqli->query("SELECT * FROM `post` ORDER BY id DESC");
							if ($results) { 
							...
							?>                        
									<tr><td><input type="checkbox">&nbsp;&nbsp;<a href="#confirm-modal" data-toggle="modal" class="trash" data-id="<?php echo $obj->id ?>" data-target="#confirm-modal"><i class="fa fa-trash-o"></i></a></td>
										<td><?php echo $s_no ?></td>
										<td><a href="?dispatch=post.update&id=<?php echo $obj->id ?>"><?php echo ucwords($obj->title) ?></a></td>
										<td><a target="_blank" href="../<?php echo $obj->urlslug;?>.html">Click Here</a></td>
										<td><?php echo $obj->SortOrder ?></td>
