		...
		$('form').submit(function(event) {
			...
			.done(function(data) {
				...
				if (! data.success) {
					...
					//alert(data.message);
					window.location = '?dispatch=events';
				}				
			})
			.fail(function(data) {
				// show any errors
					...
					<form role="form" method="post" name="update_form" id="update_form" enctype="multipart/form-data">
						<div class="box-body">
							...
							<div class="row">
								...
								<div class="col-md-6">
									<div class="form-group">
										...
										<textarea class="form-control" name="form_data[PlanningTools]" id="elm_planningtools" rows="6"></textarea>
									</div>
								</div>	
							</div>
							...
							<div class="row">
								...
								<?php if($resultsSO = $mysqli->query("SELECT * FROM `Events`")) { $SortOrderNumber = $resultsSO->num_rows+1; } ?>
