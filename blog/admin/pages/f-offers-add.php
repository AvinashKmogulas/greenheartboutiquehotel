<?php
	...
	if (! empty($errors)) {
		...
		$query = 	"INSERT INTO `SpecialOffers` ( `Title`, `Description`, `Pic`, `Inclusions`, `SortOrder`, `Status`) 
					VALUES 
					(
						...
						'" . $mysqli->real_escape_string($_POST['form_data']['Inclusions']) . "',
		...
		if ($mysqli->query($query)) {
			...
			if (!empty($_FILES["fileUpload1"]["name"]))	move_uploaded_file($_FILES["fileUpload1"]["tmp_name"], "../../assets/img/offers/".$ufile1_path);
			$data['success'] = true;
