	...
	$(document).ready(function() {
		$('#update_form').submit(function(event) {
			...
			$.ajax({
				...
				url 		: './pages/f-comment-update.php?id=<?php echo $id ?>', // the url where we want to POST
			...
			.done(function(data) {
				...
				if ( ! data.success) {
					...
					window.location = 'index.php?dispatch=comment';
				}				
			})
		...
		<div class="row">
			...
			<div class="col-md-12">
				...
				<div class="box box-primary">
					...
					<?php 
					$results = $mysqli->query("SELECT * FROM `comments` WHERE `ID` = ".$id);
							...
							<div class="row">
								<div class="col-md-6">
									<div class="form-group">
										...
										<input type="text" class="form-control" name="form_data[name]" id="elm_name" value="<?php echo $obj->name ?>" />
									</div>
								</div>
							</div>
							
							<div class="form-group">
								<label for="elm_message">Email</label>
								<input type="text" class="form-control" name="form_data[email]" id="elm_name" value="<?php echo $obj->email ?>" />
							</div>
							
							<div class="form-group">
								<label for="elm_message">Comment</label>
								<textarea class="form-control" name="form_data[comment]" id="elm_message" rows="3"><?php echo $obj->comment ?></textarea>
							</div>
								...
								<div class="col-md-4" style="display:none;">
									<div class="form-group">
										<div class="input-group">
											...
											<input type="text" class="form-control" name="form_data[SortOrder]" id="elm_sortorder" value="<?php echo $obj->SortOrder ?>" onKeyPress="return isNumber(event)" />
                  						</div>
                  					</div>
								</div>
							</div>
