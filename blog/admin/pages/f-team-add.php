<?php
	...
	if (empty($_POST['form_data']['Name'])) { $errors['sName'] = 'Name is required.'; }
	...
	if (! empty($errors)) {
		...
		$query = 	"INSERT INTO `Team` ( `Name`, `Designation`, `Description`, `Pic`, `SortOrder`, `Status`) 
					VALUES 
					(
						'" . $mysqli->real_escape_string($_POST['form_data']['Name']) . "',
						'" . $mysqli->real_escape_string($_POST['form_data']['Designation']) . "',
		...
		if ($mysqli->query($query)) {
			...
			if (!empty($_FILES["fileUpload1"]["name"]))	move_uploaded_file($_FILES["fileUpload1"]["tmp_name"], "../../assets/img/team/".$ufile1_path);
			$data['success'] = true;
